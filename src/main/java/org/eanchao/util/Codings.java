...
import org.eanchao.exception.CMSRuntimeException;
...
public abstract class Codings {
	...
	public static String urlEncode(String input) {
		try {
			...
			throw new CMSRuntimeException("不支持的编码转换格式异常 CMSRuntimeException", e);
		}
	}
	...
	static class Hex{
	    ...
	    public static byte[] decodeHex(char[] data) throws CMSRuntimeException {
	        ...
	        if ((len & 0x01) != 0) {
	            throw new CMSRuntimeException("奇数个字符。");
	        }
	    ...
	    protected static int toDigit(char ch, int index) throws CMSRuntimeException {
	        ...
	        if (digit == -1) {
	            throw new CMSRuntimeException("非法的十六进制字符 " + ch + " 。此字符在索引 " + index + " 处。");
	        }
	        return digit;
	    }
	    ...
	    public String toString() {
	        return super.toString() + "[(字符名称)charsetName=" + this.charsetName + "]";
	    }
	}
}
