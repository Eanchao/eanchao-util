 ...
 * 19、转换为参数字符串
 * 20、参数转换为Map
 * 21、以指定的分隔符来进行字符串元素连接
 * 22、以json元素的方式连接字符串中元素
 * 23、检查指定的字符串列表是否不为空,
 * 24、首字母大写（第一种实现方法）
 * 25、分割字符串得到子串
 * 26、获取子串在源串中第一次出现的位置结束, 索引从0开始的串
 * 27、获取子串在源串中最后一次出现的位置结束, 索引从0开始的串
 * 28、获取子串在源串中第一次出现的位置开始, 至源串末尾的串
 * 29、获取子串在源串中最后一次出现的位置开始, 至源串末尾的串
 * 30、获取源串中, 两个子串之间的串
 * 31、首字母大写
 * 32、首字母小写
 * 33、以参数替换占位符[?]的形式格式化字符串
 * 34、将字符串中的标点符号过滤掉
 * </pre>
 * 
 * @author Eanchao
...
public class StringUtils implements Serializable {
	...
	private static final String DEFAULT_CHART = "UTF_8";
	...
	/**
	 * 
	 * TODO URL转换为参数字符串 如 "index.jsp?id=123&code=tom"，解析为id=123&code=tom
	 * 
	 * @author MrXiao
	...
	/**
	 * 
	 * TODO URL参数转换为Map id=123&code=tom转换为Map
	 * 
	 * @author MrXiao
	...
	public static String substring(String source, int beginIndex, int endIndex) {
		...
		if (beginIndex > endIndex) {
			throw new StringIndexOutOfBoundsException("字符串索引超出范围: " + (endIndex - beginIndex));
		}
